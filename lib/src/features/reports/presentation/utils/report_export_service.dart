...
class ReportExportService {
  static Future<void> exportToCsv(ReportData data) async {
    ...
    final file = File('${directory.path}/${data.title}_${DateTime.now().millisecondsSinceEpoch}.csv');
    await file.writeAsString(csvString);
    
    // Using Share.shareXFiles and suppressing deprecation
    // ignore: deprecated_member_use
    await Share.shareXFiles([XFile(file.path)], text: 'Exported ${data.title} report');
  }
          ...
          return pw.Column(
            children: [
              ...
              pw.TableHelper.fromTextArray(
                ...
                data: data.rows.map((row) => row.map((cell) => cell.toString()).toList()).toList(),
              ),
            ],
          );
        },
